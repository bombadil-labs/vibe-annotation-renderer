...
```html
<div id="v"></div>
<script src="https://cdn.jsdelivr.net/gh/bombadil-labs/vibe-banner@97947fee867aba932f377809a186310837e3c95f/dist/vibe.min.js"></script>
<script>
...
| `scene` | the habitat, as a **framed portrait window**: a NAME — `"tidepool"` · `"study"` · `"night"` · `"park"` — and the renderer owns the art, the pin, the opacity and whether the place is alive. A custom image still takes `{ url, opacity }` (clamped 0.15–0.95); a bare word that is not a known name empties the window rather than requesting a broken image. The scene fills a rounded square on the banner's left with the face centred inside it. Every first-party scene is **live**: native ambience in the window — rising bubbles, a passing fish, tap-ripples, lamplight, feeding. The **park** goes further: it has no art file at all — sky, sun, tree and grass are drawn as live pen-strokes, a bird crosses now and then, and tapping the sun turns day to night (and back; the choice persists). **The window always draws** — with no scene it renders empty: frame and a faint interior, no view yet |
...
```js
face: {
  url: "https://cdn.jsdelivr.net/gh/bombadil-labs/vibe-banner@97947fee867aba932f377809a186310837e3c95f/assets/kip-sheet.png",
...
| `kip` | park | the project mascot (8 moods) |
...
(alive), **night**, **study** — generated by `npm run scene` — and the **park**, drawn
live with no art file at all), tune
